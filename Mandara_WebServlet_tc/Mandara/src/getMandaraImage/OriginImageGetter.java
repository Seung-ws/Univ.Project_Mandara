 ...
 */
@WebServlet("/OriginImageGetter")
public class OriginImageGetter extends HttpServlet {
	private static final long serialVersionUID = 1L;
	DataSource datafactory=null;
    /**
     * @see HttpServlet#HttpServlet()
     */
    public OriginImageGetter() {
        super();
	...
	private void ImageGetter(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if(g_uid!=null&&toName!=null&&I_type!=null)
			{
				...
				String query = "Select originImage from MandaraUserImage1 where g_uid=? AND toName=? AND I_type=?";				
